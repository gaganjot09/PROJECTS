...
void show_record()
    {system("cls");/*Every time this piece of code is processed 
	                 whatever is written in your screen will get 
					 cleared and the remaining portion of code's 
					 output if any will be displayed.*/
	char name[20];
	float scr;
	FILE *f; /*DECLARING FILE POINTER
                FILE is the data structure included in stdio.h and 
				f is file pointer which points to start of the file*/       
	f=fopen("score.txt","r");/*opening a file 
	                           fp=fopen("filenmae","mode");
							   r refers to open a text file fro reading */
	
	fscanf(f,"%s%f",&name,&scr);
	printf("\n\n\t\t*************************************************************");
	printf("\n\n\t\t %s has secured the Highest Score %0.2f",name,scr);//%0.2f is used to adjust decimals.
	printf("\n\n\t\t*************************************************************");
	fclose(f);
	getch();}
